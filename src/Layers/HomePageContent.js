    ...
    <>
      ...
      <section className="bg-specBeige">
        <div className="w-3/4 mx-auto flex flex-col items-center">
          <p className="text-specRed font-satisfy font-bold text-3xl">
            en çok paketlenen menüler
          </p>
          <h2 className="text-black font-bold text-5xl font-barlow pt-10">
            Acıktıran Kodlara Doyuran Lezzetler
          </h2>
          <div className="flex justify-between w-full mt-10">
            {dataHomePage.underBanner.map((item) => (
              <div className="flex items-center bg-white hover:bg-specDimGrey hover:text-white duration-200 justify-center w-[12%] py-4 rounded-full">
                <img src={item.img} />
                <p className="pl-5 font-semibold">{item.name}</p>
              </div>
            ))}
